...
class Environment:
    def __init__(self):
        ...
        self.slack_client_secret = os.environ.get(
            "SLACK_CLIENT_SECRET", "unset")
        self.slack_signing_secret = os.environ.get(
        ...
        unset = [key for key, value in self.__dict__.items() if value ==
                 "unset"]

        if unset:
            raise ValueError(
                f"Missing environment variables: {', '.join(unset)}")
